...
export function RegisterIncludeSource(
  includeName: string,
  includeHTML: string,
) {
  includeMap.set(includeName, includeHTML);
}
export async function LoadIncludeFile(
  includeName: string,
  includePath: string,
) {
  try {
...
async function getInclude(includeName: string) {
  ...
  if (html) {
    ...
    try {
      return Parser.parse(
        await LoadIncludeFile(includeName, includeName + ".html"),
      );
    } catch (err) {
...
export async function ProcessTemplateFile(
  ...
  await SaveHtmlFile(
    await ProcessTemplateNode(await LoadHtmlFile(templatePath)),
    outputPath,
  );
}
...
async function processInclude(oldItem: Parser.HTMLElement) {
  ...
  if (newItem !== oldItem) {
    if (newItem instanceof Parser.HTMLElement) {
      ...
      const classList = [
        ...oldItem.classList.values(),
        ...newItem.classList.values(),
      ];
